...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Lightweight KS logo animation: now SVG image, so skip text animations
    (function setupKsLogo() {
        const ksLogo = document.getElementById('ks-logo');
        if (!ksLogo || ksLogo.tagName === 'IMG' || ksLogo.querySelector('img')) return; // skip if it's an image
    ...
    heroTitle.innerHTML = 'Creating Digital Experiences'.split('').map(char => `<span>${char === ' ' ? '&nbsp;' : char}</span>`).join('');
